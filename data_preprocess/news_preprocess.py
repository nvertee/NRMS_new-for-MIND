...
def save_word_embeddings(column_words, newsid, file_name):
    ...
    f = open('/kaggle/working/NRMS_new-for-MIND/glove_dict.pkl', 'rb')
    glove = pickle.load(f)
...
def save_entity(newsdata):
    entity1 = pd.read_csv('/kaggle/working/NRMS_new-for-MIND/data/MINDsmall_train/entity_embedding.vec', header=None, sep='\t')
    entity2 = pd.read_csv('/kaggle/working/NRMS_new-for-MIND/data/MINDsmall_dev/entity_embedding.vec', header=None, sep='\t')
...
if __name__ == '__main__':
    news_train = pd.read_csv('/kaggle/working/NRMS_new-for-MIND/data/MINDsmall_train/news.tsv', sep='\t', header=None)
    news_train.columns = ['NewsID', 'Category', 'SubCategory', 'Title', 'Abstract', 'URL', 'TitleEntities', 'AbstractEntities']
    news_dev = pd.read_csv('/kaggle/working/NRMS_new-for-MIND/data/MINDsmall_dev/news.tsv', sep='\t', header=None)
